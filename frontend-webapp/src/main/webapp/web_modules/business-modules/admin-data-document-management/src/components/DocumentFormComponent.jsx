...
export class DocumentFormComponent extends React.Component {
  ...
  constructor(props) {
    ...
    if (!isCreating) {
      if (has(props.currentDocument.content, 'descriptionFile.url')) {
        showDescriptionMode = DESCRIPTION_MODE.URL
      } else if (has(props.currentDocument.content, 'descriptionFile.type')) {
  ...
  handleInitialize = () => {
    if (!this.state.isCreating) {
      ...
      const initialValues = {
        ...
        descriptionUrl: get(currentDocument.content, 'descriptionFile.url', undefined),
        properties,
      }
              ...
              <div className="col-sm-30">
                ...
                >
                  <RadioButton
                    ...
                    label={this.context.intl.formatMessage({ id: 'document.form.radio.none' })}
                    disabled={disableNoDescription}
                  />
                  <RadioButton
                    value={DESCRIPTION_MODE.FILE}
                    label={this.context.intl.formatMessage({ id: 'document.form.radio.descriptionFileContent' })}
                  />
                  <RadioButton
                    value={DESCRIPTION_MODE.URL}
                    label={this.context.intl.formatMessage({ id: 'document.form.radio.descriptionUrl' })}
                  />
                </RadioButtonGroup>
              </div>
              <div className="col-sm-70">
                <ShowableAtRender show={showDescriptionMode === DESCRIPTION_MODE.URL}>
                  <Field
                    ...
                    component={RenderTextField}
                    type="text"
                    label={this.context.intl.formatMessage({ id: 'document.form.descriptionUrl' })}
                  />
                </ShowableAtRender>
                <ShowableAtRender show={showDescriptionMode === DESCRIPTION_MODE.FILE}>
                  <ShowableAtRender show={!disableNoDescription}>
                    <FormattedMessage id="document.form.descriptionFileContent" />
                  </ShowableAtRender>
                  <ShowableAtRender show={disableNoDescription}>
                    <FormattedMessage id="document.form.descriptionFileContentReuploadToOverride" />
                  </ShowableAtRender>
                  <Field
