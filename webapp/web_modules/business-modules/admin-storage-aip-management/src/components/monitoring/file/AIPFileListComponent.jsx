...
export class AIPFileListComponent extends React.Component {
  ...
  getDialogActions = () => [
    <FlatButton
      key="cancel"
      label={this.context.intl.formatMessage({ id: 'aips.files.modal.action.close' })}
      primary
  ...
  renderStacktraceDialog = () => (
    ...
    >
      <FitContentDialog
        title={this.context.intl.formatMessage({ id: 'aips.files.modal.title' })}
        modal
  ...
  renderBreadCrump = () => {
    ...
    const elements = [
      formatMessage({ id: 'aips.session.title' }),
      formatMessage({ id: 'aips.session.aips.title' }, { session }),
      formatMessage({ id: 'aips.files.title' }),
    ]
    return (
      <Breadcrumb
        rootIcon={<PageView />}
  ...
  render() {
    ...
    const columns = [
      new TableColumnBuilder('column@name').titleHeaderCell().propertyRenderCell('content.name')
        .label(formatMessage({ id: 'aips.files.table.headers.name' }))
        .build(),
      new TableColumnBuilder('column@state').titleHeaderCell()
        ...
        .label(formatMessage({ id: 'aips.files.table.headers.state' }))
        .build(),
      new TableColumnBuilder('column@type').titleHeaderCell().propertyRenderCell('content.dataType')
        .label(formatMessage({ id: 'aips.files.table.headers.type' }))
        .build(),
      new TableColumnBuilder('column@filesize').titleHeaderCell().propertyRenderCell('content.fileSize', StorageCapacityRender)
        .label(formatMessage({ id: 'aips.files.table.headers.filesize' }))
        .build(),
    ]

    const emptyComponent = (
      <NoContentComponent
        title={formatMessage({ id: 'aips.files.table.empty.title' })}
        Icon={AddToPhotos}
      />
    )
...
export default withModuleStyle(styles)(withI18n(messages)(AIPFileListComponent))
