...
class AIPDeletionErrorDialog extends React.Component {
  static propTypes = {
    ...
    aipId: PropTypes.string, // provided only when there are errors
  ...
  render() {
    const { errors, aipId } = this.props
    const { intl: { formatMessage } } = this.context
    return (
      <Dialog
        title={formatMessage({ id: 'aip.delete.error.title' }, { id: aipId })}
...
export default withI18n(messages)(AIPDeletionErrorDialog)
