...
class ConfirmDeleteOnSomeStoragesDialogComponent extends React.Component {
  ...
  render() {
    ...
    // compute list label
    const datastorages = selectedStorages.map(({ dataStorageConfiguration: { label } }) => label).join(formatMessage({ id: 'aip.delete.on.selected.storages.label.separator' }))
    return (
      <Dialog
        ...
        open
      >
        {formatMessage({ id: 'aip.delete.on.selected.storages.messages' }, { datastorages }) }
      </Dialog>
