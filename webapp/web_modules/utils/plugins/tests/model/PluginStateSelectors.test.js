...
describe('[PLUGINS] Testing initializePluginSelectors', () => {
  ...
  it('should select correctly state as it changes changes', () => {
    ...
    fakeStore = mockReduce(fakeStore, actions.publishState('plugin1', { name: 'test1' }, 'query1'))
    assert.deepEqual(selectors.getCriterionData(fakeStore, 'plugin1'), { state: { name: 'test1' }, query: 'query1' }, 'A - Plugin 1 state should be retrieved')
    assert.isNotOk(selectors.getCriterionData(fakeStore, 'plugin2'), 'A - Plugin 2 state should not be retrieved')

    fakeStore = mockReduce(fakeStore, actions.publishState('plugin2', { anything: 2 }, 'query2'))
    assert.deepEqual(selectors.getCriterionData(fakeStore, 'plugin1'), { state: { name: 'test1' }, query: 'query1' }, 'B - Plugin 1 state should be retrieved')
    assert.deepEqual(selectors.getCriterionData(fakeStore, 'plugin2'), { state: { anything: 2 }, query: 'query2' }, 'B - Plugin 2 state should be retrieved')

    fakeStore = mockReduce(fakeStore, actions.publishAllStates({
      plugin2: { state: { name: 'plugin2' }, query: 'query2' },
      plugin3: { state: { name: 'plugin3' }, query: 'query3' },
    }))
    assert.isNotOk(selectors.getCriterionData(fakeStore, 'plugin1'), 'C - Plugin 1 state should have been removed')
    assert.deepEqual(selectors.getCriterionData(fakeStore, 'plugin2'), { state: { name: 'plugin2' }, query: 'query2' }, 'C - Plugin 2 should be updated')
    assert.deepEqual(selectors.getCriterionData(fakeStore, 'plugin3'), { state: { name: 'plugin3' }, query: 'query3' }, 'C - Plugin 3 should be created')
