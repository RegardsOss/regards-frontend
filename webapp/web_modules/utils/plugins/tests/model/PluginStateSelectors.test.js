...
describe('[PLUGINS] Testing initializePluginSelectors', () => {
  ...
  it('should select correctly state as it changes changes', () => {
    ...
    assert.deepEqual(selectors.getCriterionData(fakeStore, 'plugin1'), { state: { name: 'test1' }, queryParameters: { q: 'query1' } }, 'A - Plugin 1 state should be retrieved')
    ...
    assert.deepEqual(selectors.getCriterionData(fakeStore, 'plugin1'), { state: { name: 'test1' }, queryParameters: { q: 'query1' } }, 'B - Plugin 1 state should be retrieved')
    assert.deepEqual(selectors.getCriterionData(fakeStore, 'plugin2'), { state: { anything: 2 }, queryParameters: { q: 'query2' } }, 'B - Plugin 2 state should be retrieved')

    fakeStore = mockReduce(fakeStore, actions.publishAllStates({
      plugin2: { state: { name: 'plugin2' }, queryParameters: { q: 'query2' } },
      plugin3: { state: { name: 'plugin3' }, queryParameters: { q: 'query3' } },
    }))
    assert.isNotOk(selectors.getCriterionData(fakeStore, 'plugin1'), 'C - Plugin 1 state should have been removed')
    assert.deepEqual(selectors.getCriterionData(fakeStore, 'plugin2'), { state: { name: 'plugin2' }, queryParameters: { q: 'query2' } }, 'C - Plugin 2 should be updated')
    assert.deepEqual(selectors.getCriterionData(fakeStore, 'plugin3'), { state: { name: 'plugin3' }, queryParameters: { q: 'query3' } }, 'C - Plugin 3 should be created')
