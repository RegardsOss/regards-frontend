...
describe('[PLUGINS API] Testing NumberRange', () => {
  ...
  it('should correctly generate query parts for numbers', () => {
    ...
    assert.isNotOk(NumberRange.getNumberQueryParameter(null, new NumberRange.NumberRange(5, null)).toQueryString(), 'It should not return query without attribute name')
    assert.isNotOk(NumberRange.getNumberQueryParameter('my.attr', null).toQueryString(), 'It should not return query without range')
    assert.isNotOk(NumberRange.getNumberQueryParameter('my.attr', new NumberRange.NumberRange()).toQueryString(),
    ...
    assert.equal(NumberRange.getNumberQueryParameter('my.attr', new NumberRange.NumberRange(-0.125, -0.125)).toQueryString(),
      'my.attr:\\-0.125')
    assert.equal(NumberRange.getNumberQueryParameter('my.attr', new NumberRange.NumberRange(-5, 88)).toQueryString(),
      'my.attr:[\\-5 TO 88]')
    assert.equal(NumberRange.getNumberQueryParameter('other.attr', new NumberRange.NumberRange(null, 4)).toQueryString(),
      'other.attr:[* TO 4]')
    assert.equal(NumberRange.getNumberQueryParameter('other.attr', new NumberRange.NumberRange(-1.25, null)).toQueryString(),
      'other.attr:[\\-1.25 TO *]')
  })
  it('should correctly define number ranges', () => {
    // A - single value range
    const singleValueRange = new NumberRange.NumberRange(1.25, 1.25)
  ...
  // TODO intersects, isValidRestriction and isEmpty tests
})
