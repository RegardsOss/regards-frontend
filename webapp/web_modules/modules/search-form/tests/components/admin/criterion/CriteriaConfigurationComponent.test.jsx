...
describe('[SEARCH FORM] Testing CriteriaConfigurationComponent', () => {
  ...
  it('Should render a CriteriaConfigurationComponent', () => {
    ...
    // note: 6 selectable attributes are expected for plugin attributeType ["String"] (2 from above properties, 5 from standard
    // parameters)
    assert.lengthOf(attributes.at(0).find(MenuItem), 7, 'There should be available attributes (filtered by type using plugin info)')
    assert.lengthOf(attributes.at(1).find(MenuItem), 7, 'There should be available attributes (filtered by type using plugin info)')
  })
})
