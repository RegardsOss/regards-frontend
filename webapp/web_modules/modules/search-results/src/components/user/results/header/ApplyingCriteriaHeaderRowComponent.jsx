...
import { TableHeaderLine, TableHeaderContentBox, TableHeaderOptionsSeparator } from '@regardsoss/components'
...
class ApplyingCriteriaHeaderRowComponent extends React.Component {
  ...
  render() {
    const {
      facetValues, geometries, entitiesSelections,
      onUnselectFacetValue, onUnselectGeometry, onUnselectEntitiesSelection,
    } = this.props
    return (
      <ShowableAtRender show={facetValues.length > 0 || geometries.length > 0 || entitiesSelections.length > 0}>
        <TableHeaderLine>
