...
import SingleSortingComponent, { SingleSortingModelEnum } from '../../../../../components/user/results/header/options/SingleSortingComponent'
...
export class SingleSortingContainer extends React.Component {
  ...
  onPropertiesChanged = (oldProps, newProps) => {
    ...
    const { resultsContext: { type, typeState } } = newProps
    const {
      enableSorting, isInInitialSorting, initialSorting, criteria: { sorting: currentSorting }, mode, modeState,
    } = typeState[type]
    const { presentationModels } = modeState[mode]
  ...
  onSortBy = (sortingModel) => {
    if (!sortingModel.selected) { // only when not already selected
      ...
      const { moduleId, updateResultsContext, resultsContext: { type } } = this.props
