...
import ResultFacetsHeaderRowComponent from '../../../../components/user/results/header/ResultFacetsHeaderRowComponent'
...
export class ResultFacetsHeaderRowContainer extends React.Component {
  ...
  onPropertiesUpdated = (oldProps, newProps) => {
    const { resultsContext: { type, typeState }, facets: resultsFacets } = newProps
    const newFacetsState = typeState[type].facets
    const oldType = get(oldProps, 'resultsContext.type')
    const oldFacetsState = get(oldProps, `resultsContext.typeState.${oldType}.facets`)

    // when results facets or current type state facets changed, update resolved facets (store them in state to avoid computing it
    // at render time
    if (!isEqual(oldProps.facets, resultsFacets) || !isEqual(oldFacetsState, newFacetsState)) {
      this.setState({
        facets: ResultFacetsHeaderRowContainer.buildFacetModels(resultsFacets, newFacetsState.list),
      })
    }
  }
  ...
  render() {
    ...
    return (
      <ResultFacetsHeaderRowComponent
        ...
        facetsEnabled={currentTypeState.facets.enabled && facets.length > 0}
        facets={facets}
