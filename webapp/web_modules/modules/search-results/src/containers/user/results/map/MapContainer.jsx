...
import MapComponent from '../../../../components/user/results/map/MapComponent'
...
export class MapContainer extends React.Component {
  ...
  onPropertiesUpdated = (oldProps, newProps) => {
    ...
    const { entities: oldEntities, resultsContext: oldResultsContext } = oldProps
    const { entities, pageMetadata = { number: 0, size: MapComponent.MAP_PAGE_SIZE }, resultsContext } = newProps
    ...
    const { currentModeState: { selectionMode } } = UIDomain.ResultsContextConstants.getViewData(resultsContext)
    const { currentModeState: { selectionMode: oldSelectionMode } } = UIDomain.ResultsContextConstants.getViewData(oldResultsContext)
    ...
    if (!isEqual(get(oldResultsContext, 'criteria.geometry'), resultsContext.criteria.geometry)) {
      nextState.criteriaAreas = resultsContext.criteria.geometry.map(
  ...
  onSetSelectionMode = (selectionMode) => {
    ...
    const { type, mode, currentModeState } = UIDomain.ResultsContextConstants.getViewData(resultsContext)
    // update only when there is some change
    if (selectionMode !== currentModeState.selectionMode) {
      // update in results context, by diff
      updateResultsContext(moduleId, {
        typeState: {
          [type]: {
            modeState: {
              [mode]: { // necessarily map here
                selectionMode,
              },
            },
          },
  ...
  onDrawingSelectionDone = (point1, point2) => {
    const { moduleId, updateResultsContext, resultsContext } = this.props
    const { type, mode } = UIDomain.ResultsContextConstants.getViewData(resultsContext)
      ...
      updateResultsContext(moduleId, {
        typeState: {
          [type]: {
            modeState: {
              [mode]: {
              // back to default selection mode
  ...
  render() {
    ...
    const { currentModeState: { backgroundLayer, selectionMode } } = UIDomain.ResultsContextConstants.getViewData(resultsContext)
    return (
      <MapComponent
