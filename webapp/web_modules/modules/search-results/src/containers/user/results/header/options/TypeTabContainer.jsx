...
import TypeTabComponent from '../../../../../components/user/results/header/options/TypeTabComponent'

/**
 * Container for tab type selector (to switch current view entities type)
...
export class TypeTabContainer extends React.Component {
  ...
  onTypeSelected = () => {
    const {
      moduleId, type, resultsContext, updateResultsContext,
    } = this.props
    if (resultsContext.type !== type) {
      // update state by diff (only set the new type field value)
      updateResultsContext(moduleId, { type })
    }
  ...
  render() {
    const { type, resultsContext } = this.props
    return (
      <TypeTabComponent type={type} resultsContext={resultsContext} onTypeSelected={this.onTypeSelected} />
    )
  }
}
