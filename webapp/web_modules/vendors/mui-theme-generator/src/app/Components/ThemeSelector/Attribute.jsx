...
export const createAttributes = (object, overwrites, addToOverwrites, removeFromOverwrites, resetOverwrite, overwriteSelectorParent = [], allowRemove, allowReset) => {
  ...
  const overwriteParent = overwriteSelectorParent.reduce((result, selector) => result[selector] || {}, overwrites)

  return Object.keys(object).map((key) => {
    let children; let right; let
      left
    const value = object[key]
    const type = typeof value
    const secondaryText = type === 'string' ? value : null
    const overwriteSelector = [...overwriteSelectorParent, key]
      ...
      case 'string': //Color or text
        if (!value.match(regexColor)) {
          ...
          return <div
            key={key}
            style={{
              position: 'relative', marginLeft: 18 * (overwriteSelector.length), paddingLeft: 72, paddingRight: 56
            }}
          >
