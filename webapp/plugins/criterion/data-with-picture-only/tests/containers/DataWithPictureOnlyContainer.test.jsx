...
import { DataWithPictureOnlyContainer } from '../../src/containers/DataWithPictureOnlyContainer'
import DataWithPictureOnlyComponent from '../../src/components/DataWithPictureOnlyComponent'
import styles from '../../src/styles/styles'

const context = buildTestContext(styles)

/**
 * Test DataWithPictureOnlyContainer
 *
 * @author Raphaël Mechali
 */
describe('[DATA WITH PICTURE ONLY criterion] Testing DataWithPictureOnlyContainer', () => {
  before(testSuiteHelpers.before)
  after(testSuiteHelpers.after)
  it('should exists', () => {
    assert.isDefined(DataWithPictureOnlyContainer)
  })
  it('should render correctly without initial state', () => {
    ...
    const enzymeWrapper = shallow(<DataWithPictureOnlyContainer {...props} />, { context })
    const componentWrapper = enzymeWrapper.find(DataWithPictureOnlyComponent)
    assert.lengthOf(componentWrapper, 1, 'There should be the component')
    testSuiteHelpers.assertWrapperProperties(componentWrapper, {
      label: props.label,
      checked: DataWithPictureOnlyContainer.DEFAULT_STATE.checked,
  ...
  it('should render self and sub components with initial state, publishing state and parameters on updates', () => {
    ...
    const enzymeWrapper = shallow(<DataWithPictureOnlyContainer {...props} />, { context })
    let componentWrapper = enzymeWrapper.find(DataWithPictureOnlyComponent)
    ...
    componentWrapper = enzymeWrapper.find(DataWithPictureOnlyComponent)
    ...
    assert.deepEqual(spiedPublishState.nextParameters, {
      [CatalogDomain.CatalogSearchQueryHelper.HAS_IMAGE_PARAMETER_NAME]: 'true',
    }, '2 - Only data with picture request should be built')
  })
})
