 ...
 * @author Léo Mieulet
 */
const pluginName = 'fem-notify'
/**
 * the store key used in reducer.js
 */
const storeKey = 'recipients'

const actionsBuilder = (namespace) => new AccessProjectClient.RecipientsActions(namespace)
const reducerBuilder = (namespace) => AccessProjectClient.getRecipientsReducer(namespace)
const selectorsBuilder = (storePath) => AccessProjectClient.getRecipientSelectors(storePath)
...
export function getRecipientsClient(pluginInstanceId) {
  return clientInfoBuilder.getClient(pluginInstanceId)
}
// Expose requests reducer
export function getRecipientsReducer(pluginInstanceId) {
